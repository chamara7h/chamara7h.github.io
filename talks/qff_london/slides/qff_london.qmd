...
::: {.column style="flex: 0 0 50%;"}

### `Human story`: What data misses

<br>

`Nilu` went to a pharmacy for Product A. It was [not in stock]{.highlight-blue-big} and the system logs it as [zero demand.]{.highlight-blue-big}

<br>

When data is censored by stockouts or service interruptions...
[Forecasts fail...]{.highlight-blue-big}  Not just by being wrong, but by being [blind.]{.highlight-blue-big}

<br>

This creates broken trust and leads to [UNMET DEMAND.]{.highlight-blue-big}
...
Contraceptives aren't easily substitutable. A lost sale = [a lost opportunity for care.]{.highlight-blue-big}
...
::: {.r-stack}
![](images/tkf_cp_framework_1.png){.fragment width=45%}

![](images/tkf_cp_framework_2.png){.fragment width=45%}

![](images/tkf_cp_framework_3.png){.fragment width=45%}

![](images/tkf_cp_framework_4.png){.fragment width=45%}

![](images/tkf_cp_framework_5.png){.fragment width=45%}

![](images/tkf_cp_framework_6.png){.fragment width=45%}
...
layout(lm, widths = c(1,1), heights = c(1,1))

# 3) Plot margins & font
par(mar = c(3, 3, 2, 1), oma = c(0, 0, 0, 0), family = "Assistant", cex = 2.25)
...
plot_df <- num_ex_metrics |>
  ...
  filter(!model %in% c("Naive", "Linear Regression")) |> 
  pivot_longer(
...
::: {.column style="flex: 0 0 50%; display: flex; flex-direction: column; justify-content: left; text-align: left;"}
Develop a quantile-based inventory policy
