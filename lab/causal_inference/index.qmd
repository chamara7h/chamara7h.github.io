...
description: "A hands-on tutorial on Double Lasso and Double Machine Learning in R."
...
<div class="hero-section">
  ...
  <p class="description">A hands-on tutorial on Double Lasso and Double Machine Learning in R.</p>
  <p class="author-date">Harsha Chamara Hewage | 2026-02-13</p>
  <div class="cta-buttons">
    <a href="https://github.com/chamara7h/causal_inference_with_machine_learning_tutorial" target="_blank" class="btn-custom btn-secondary">
...
::: {.content-section .tutorial-content}
...
This idea is closely related to the **Convergence Hypothesis** predicted by the Slow Growth Model. This is a structural economic model. Under some strong assumptions (not discussed here), this predictive exercise can be given a causal interpretation.
...
```{r eval=FALSE}
install.packages(c("tidyverse", "hdm", "broom"))
```
...
**Exercise 1:** To check the convergence hypothesis, analyze the relationship between the country's growth rate $Y$ and the country's other characteristics by running a linear regression (ols) in the first step. Determine the regression coefficient $\beta_1$ of the target regressor *gdpsh465* (initial wealth level, $D$), its 95% confidence interval and the standard error.
...
ols_key |>
  transmute(
    Term = "Initial GDP per capita (gdpsh465)",
...
The sample contains about 90 countries and about 60 variables, so $p/n$ is not small. In this setting, the least squares method may provide a noisy estimate of $\beta_1$, especially when many controls are weakly informative.
...
```{r eval=FALSE}
install.packages(c("mlr3", "mlr3learners", "DoubleML", "glmnet", "ranger", "xgboost", "data.table"))
```
